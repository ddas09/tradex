...
def stocks(request):
    ...
    start = requested_page - 2 if (requested_page - 2) > 0 else 1
    end = requested_page + 2 if (requested_page + 2) < content.num_pages else content.num_pages 
...
def sell(request):
    ...
    if request.method == "POST":
        ...
        if shares > 100:
            return JsonResponse({"status": 1, "alertInputId": "#shares", "msg": "Enter a number less than or equal to 100"})        
        
        shares_owned = stock["shares_owned"]
        if shares <= shares_owned:
            stock_price = getStockPrice(symbol)
            gross_income = shares * stock_price

            # Apply brokerage according to user plan
            net_income = deductBrokerage(user_id, gross_income)
            
            updateUserBalance(user_id, net_income)
