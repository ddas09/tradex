...
def deductBrokerage(user_id, gross_income):
    query = "SELECT plan_type FROM users WHERE id = %s"
    result = executeQuery(query, [user_id])
    
    plan = result[0]['plan_type']
    # Apply charge according to plan type
    if plan == 'S':
        return gross_income - (gross_income / 100)

    elif plan == 'P':
        return gross_income - ((gross_income * 0.8) / 100)    

    else:
        return gross_income - ((gross_income * 0.5) / 100)    
